create function public.select_bkt_routine_variant(
...
begin
  ...
  if not personalization_enabled then
  ...
  elsif observation_count < 4 or eligible_session_count < 3 or eligible_day_count < 2 then
    resolved_reason := 'insufficient_game_sessions';
    resolved_explanation :=
      'Rutin düzeyi için en az üç oturumun iki farklı güne yayılması bekleniyor.';
  else
    ...
    if resolved_personalized and (
      ...
      resolved_explanation :=
        'Birden fazla güne yayılan rutin denemelerinde destek gerektiği için daha sakin bir onaylı düzey öne alındı.';
      ...
      resolved_explanation :=
        'Birden fazla güne yayılan rutin denemeleri bağımsız tamamlandığı için sonraki onaylı düzey öne alındı.';
      supporting_count := correct_observation_count;
    else
      resolved_reason := 'general_rotation';
      resolved_explanation :=
        'Birden fazla güne yayılan rutin denemelerine göre mevcut onaylı düzey korunuyor.';
...
revoke all on function public.select_bkt_routine_variant(uuid, text, text) from public, anon;
grant execute on function public.select_bkt_routine_variant(uuid, text, text) to authenticated;

comment on function public.select_bkt_routine_variant(uuid, text, text) is
  'Applies the approved routine BKT model to consented per-round correctness evidence; requires three sessions across two days and changes at most one approved level.';
