...
add constraint game_draft_identity_matches check (
  ...
  and game->>'mechanic' in ('tap_or_wait', 'classify_and_sort')
);
...
begin
  ...
  if candidate_game->>'mechanic' not in ('tap_or_wait', 'classify_and_sort') then
...
comment on function public.save_game_draft(jsonb, uuid) is
'Validates the shared game envelope and saves supported manual, AI, or automated mechanics as a versioned draft.';
